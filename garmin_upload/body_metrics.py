# https://raw.githubusercontent.com/wiecosystem/Bluetooth/master/sandbox/huami.health.scale2/body_metrics.py
...
class bodyMetrics:
    ...
    def getFatPercentage(self):
        ...
        if self.sex == 'male' and self.weight < 61:
            ...
            coefficient = 1.0
        fatPercentage = (
            1.0 - (((LBM - const) * coefficient) / self.weight)) * 100
    ...
    def getMuscleMass(self):
        muscleMass = self.weight - \
            ((self.getFatPercentage() * 0.01) * self.weight) - self.getBoneMass()
    ...
    def getVisceralFat(self):
        if self.sex == 'female':
            if self.weight > (13 - (self.height * 0.5)) * -1:
                subsubcalc = ((self.height * 1.45) +
                              (self.height * 0.1158) * self.height) - 120
                subcalc = self.weight * 500 / subsubcalc
                vfal = (subcalc - 6) + (self.age * 0.07)
            else:
                subcalc = 0.691 + (self.height * -0.0024) + \
                    (self.height * -0.0024)
                vfal = (((self.height * 0.027) - (subcalc * self.weight))
                        * -1) + (self.age * 0.07) - self.age
        else:
            if self.height < self.weight * 1.6:
                subcalc = ((self.height * 0.4) -
                           (self.height * (self.height * 0.0826))) * -1
                vfal = ((self.weight * 305) / (subcalc + 48)) - \
                    2.9 + (self.age * 0.15)
            else:
                subcalc = 0.765 + self.height * -0.0015
                vfal = (((self.height * 0.143) - (self.weight * subcalc))
                        * -1) + (self.age * 0.15) - 5.0
    ...
    def getProteinPercentage(self, orig=True):
        ...
        if orig:
            ...
            proteinPercentage = 100 - \
                (floor(self.getFatPercentage() * 100) / 100)
            proteinPercentage -= floor(self.getWaterPercentage() * 100) / 100
            proteinPercentage -= floor((self.getBoneMass() /
                                       self.weight*100) * 100) / 100
    ...
    def getMetabolicAge(self):
        if self.sex == 'female':
            metabolicAge = (self.height * -1.1165) + (self.weight * 1.5784) + \
                (self.age * 0.4615) + (self.impedance * 0.0415) + 83.2548
        else:
            metabolicAge = (self.height * -0.7471) + (self.weight * 0.9161) + \
